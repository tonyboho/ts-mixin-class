...
console.log("mixedin | basic:", new Combined().chain())
...
console.log("mixedin | deep: ", new DCombined().step())
...
// ── instanceof (does the composed instance pass instanceof against its mixins?) ──
{
    ...
    console.log("mixedin | instanceof:", ok.length === 3 ? "✅ all" : ok.length ? `partial (${ok.join(",")})` : "❌ none")
}
