...
| 1.33 | **`this`-typed accessors** on a mixin (`get self(): this`, nested shapes like `get pair(): [this, string]`, a this-typed setter): fall back to a PROPERTY signature in the generated interface — narrowing at the consumer is identical — because a `this` type inside an INTERFACE accessor crashes the checker in plain TypeScript 6.0 (a regression: 5.9.3 clean, 6.0.3 + nightly crash; see TODO.md "Upstream") | ✅ | `mixin-accessor-edges.t.ts` |
| 1.34 | A **SYMBOL-named accessor** (`get [Symbol.toStringTag]()`): a typed member of the consumer (§1.17's accessor twin) | ✅ | `mixin-accessor-edges.t.ts` |
...
| 2.17 | A consumer accessor override reaching the mixin's accessor **through `super`** (`super.value` get AND set) — legal only because BOTH planes give the pre-transform base-less consumer a base | ✅ | `mixin-accessor-edges.t.ts` |
| 2.18 | **OVERLOADED constructors**: on a consumer (the synthetic `super()` lands only in the implementation) and on a mixin (the overload signatures survive the factory wrap) | ✅ | `member-shapes-edges.t.ts` |
| 2.19 | A consumer's **own PRIVATE surface** is untouched (only MIXIN members must be public — §11.1): `#private`, `private`/`protected` members, and a `private constructor` + static factory (singleton) through the heritage rewrite, both planes | ✅ | `member-shapes-edges.t.ts` |
...
| 7.21 | A **static helper calling the generated `.new` from inside the class body** (`static make(name) { return Job.new({ name }) }`) — on a construction class AND a construction mixin | ✅ | `member-shapes-edges.t.ts` |
