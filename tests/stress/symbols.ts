...
export type SymbolSite = {
    ...
    // references — see `inClassExtends`.
    inHeritageClause : boolean,
    // True when the identifier sits in the EXTENDS clause of ANY class — consumers,
    // plain classes and `@mixin` classes alike (the navigable-base fast path covers
    // a well-typed mixin's own required-base heritage too). These positions must
    // resolve — an empty references result there is a regression.
    inClassExtends   : boolean,
    ...
    const visit = (node: ts.Node): void => {
        if (ts.isIdentifier(node) && node.text.length > 0) {
            ...
            sites.push({
                ...
                inClassExtends   : isInClassExtendsClause(node),
...
function isInClassExtendsClause(node: ts.Node): boolean {
    const clause = ts.findAncestor(node, (ancestor) => ts.isHeritageClause(ancestor))

    return clause !== undefined &&
        clause.token === ts.SyntaxKind.ExtendsKeyword &&
        ts.isClassDeclaration(clause.parent)
}
