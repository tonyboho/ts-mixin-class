...
| a generated helper import goes missing (TS2304 on `__X__`) or a consumed decorator import survives pruning (TS6133), after adding a new import-management consumer | `collectReferencedIdentifierNames` answers ONLY for the names in `referencedNameQueries` (helper candidates + decorator bindings, `generated-imports.ts`) — it no longer collects every identifier. A new membership test against `referencedNames` needs its names added to the query set. |
