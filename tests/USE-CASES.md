...
| 2.20 | **MEMBER decorators** (standard TC39): on a CONSUMER's members — preserved, run ONCE; on a MIXIN's members — ride inside the factory class expression and run PER APPLICATION (canonical + each base-less consumer — the §1.18 static-block semantics). LEGACY (`experimentalDecorators`) limitation: a mixin's member decorators are impossible (legacy decorators are invalid on class-EXPRESSION members — TS1206, correctly spanned); consumer member decorators work in both modes | ✅ | `member-decorators.t.ts`, `fixture-suite/src/member-decorators.t.ts` (standard plane; excluded from the legacy build) |
