...
| 7.17 | A **public PARAMETER PROPERTY** on a construction class's own constructor (`constructor(public tag: string = …)`) is a `.new` config key (OPTIONAL — the constructor default supplies a value first, `Object.assign` overrides), present in the generated `<Class>Config` alias | ✅ | `construction-parameter-property.t.ts` |
